...
-you can see game(s) you are currently playing in https://steamcommunity.com/id/your_steam_name/, 'Currently In-Game', press f5 to refresh from time to time.  Steamcommunity will show just a single game running, although multiple games will be running (if you selected them in ID)  
-In Idle Daddy, in 'games' page in ID, pull down to refresh (after deselecting games in Idle Daddy)  
-steam app will not mark games current running as 'running', but their game play time will increase (after deselecting them in ID)  
-Running steam app does not make the difference, unless you are running 32 games in ID, and want to play another one. Steam can track up to 32 games running at once.  
  
-if you still get logged out out of ID: disable App battery optimization (set 'Battery optimization' to not optimized). If it does not help, disable phone 'battery save' mode.  
